	...
	while(1) {
		...
		if(FD_ISSET(master_sock_tcp_fd, &readfds)) {
			...
			while(1) {
				...
				Student student_recieved;

				memset(&student_recieved, 0, sizeof(Student));
				...
				sent_recv_bytes = recvfrom(comm_socket_fd, &student_recieved, sizeof(Student), 0,
				 ...
				 * for forever*/
				/*Step 9 */
				if(student_recieved.age == 0 && student_recieved.group == 0) {
				...
				char response[128];
				sprintf(response, "%s, %hhu y.o., group %hhu",
						student_recieved.name, student_recieved.age, student_recieved.group);

				/* Server replying back to client now*/
				sent_recv_bytes = sendto(comm_socket_fd, response, sizeof(char) * 128, 0,
