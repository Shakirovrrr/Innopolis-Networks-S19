...
void setup_tcp_communication() {
	...
	while(1) {
		...
		Student student_to_send;

		printf("Enter student name: ");
		scanf("%s", student_to_send.name);
		printf("Enter student age: ");
		scanf("%hhu", &student_to_send.age);
		printf("Enter student group: ");
		scanf("%hhu", &student_to_send.group);
		...
		sent_recv_bytes =  recvfrom(sockfd, response, sizeof(char) * 128, 0,
